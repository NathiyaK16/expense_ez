...
//     { label: 'All', value: 'all' },
//   ];
//   const [policyDetailsSec, setPolicyDetailsSec] = useState([]);

  
//   const combinedPolicyDetails = [...policyDetails, ...policyDetailsSec];
...
//       const response = await axios.get(`${BASEPATH}v1/client/ocr_inserts/get_all_claims/?emp_id=EMP002&company_id=durr`);
//       setPolicyDetails(response.data.policy_details_data || []);
//       setPolicyDetailsSec(response.data.policy_details_data_sec || []);
//     } catch (error) {
//       console.error('Error fetching policy details:', error);
//     }
//   };
...
//   //   if (!Array.isArray(policyDetails) || policyDetails.length === 0) return 'N/A';
//   //   const match = policyDetails.find(
//   //     (p) => p.policy_detail_id === policyId && p.main_expense_head === expenseId
//   //   );
//   //   return match ? match.expense_head_name || 'N/A' : 'N/A';
//   // };
...
//   //   );
//   //   return match ? match.sub_expense_name || 'N/A' : 'N/A';
//   // };

//   const getMainExpenseName = (policyId, expenseId) => {
//     const match = combinedPolicyDetails.find(
//       (p) => p.policy_detail_id === policyId && p.main_expense_head === expenseId
//     );
//     return match ? match.expense_head_name || match.main_expense_head_name || 'N/A' : 'N/A';
//   };
  
//   // UPDATED: Get sub expense name
//   const getSubExpenseName = (policyId, subExpenseId) => {
//     const match = combinedPolicyDetails.find(
//       (p) => p.policy_detail_id === policyId && p.sub_expense_head === subExpenseId
//     );
//     return match ? match.sub_expense_name || match.sub_expense_head_name || 'N/A' : 'N/A';
//   };
//   const renderClaim = ({ item }) => {
//     return (
//       <TouchableOpacity
//         style={[styles.claimItem, { backgroundColor: theme.cardBg }]}
//         onPress={() => { /* Add your claim press handler here */ }}>
//         <View style={styles.claimContent}>
//           <View style={styles.claimHeader}>
//             <Text style={[styles.categoryText, { color: theme.text }]}>
//               Claim #{item.claim_id}
...
//           </Text> */}
//           <Text style={{ color: theme.text }}>
//   Main Category: {item.main_expense_name}
// </Text>
// <Text style={{ color: theme.text }}>
//   Sub Category: {item.sub_expense_name}
// </Text>
//           <Text style={[styles.dateText, { color: theme.text }]}>
//             {item.submitted_date}
//           </Text>
...
//       <View style={styles.filterRow}>        <View style={styles.dropdownWrapper}>
//           <DropDownPicker
//              open={dateOpen}
//              value={null}
//              items={dateItems}
...
const ClaimsScreen = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Set claims and policy details from the same response
      setClaims(response.data.claims);
      setAllClaims(response.data.claims); // Assuming this stores the same data as 'claims'
      setPolicyDetails(response.data.policy_details_data); // Store policy details
  ...
  const getMainExpenseName = (policyId, expenseId) => {
    const combinedPolicyDetails = [...(policyDetails || [])];
    if (combinedPolicyDetails.length === 0) return 'N/A';
    const match = combinedPolicyDetails.find(
      (p) => p.policy_detail_id === policyId && p.main_expense_head === expenseId
    );
    return match ? match.expense_head_name || 'N/A' : 'N/A';
